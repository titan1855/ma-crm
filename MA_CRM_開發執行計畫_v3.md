...
└── users/{userId}/                    ← Firebase Auth UID，每個使用者獨立空間
    ...
    │   │   }
    │   ├── customTags: {              // 使用者自訂的標籤（產品見證用）
    │   │     issues: [string],        // 自訂問題分類（merge 到預設清單後面）
    │   │     products: [string]       // 自訂產品標籤（merge 到預設清單後面）
    │   │   }
    │   └── streak: {
    ...
    │       └── updatedAt: timestamp
    │
    ├── [collection] testimonials/     ← 模組十一：產品見證
    │   └── {testimonialId}/
    │       ├── issues: [string]        // 改善問題標籤（多選）
    │       ├── products: [string]      // 使用產品標籤（多選）
    │       ├── content: string         // 見證內容
    │       ├── source: string          // 來源（名字 / 空白）
    │       ├── duration: string        // 使用時間（例如 "2週"）
    │       ├── photos: [string]        // Base64 壓縮圖片（0~5 張，每張 ≤ 150KB）
    │       └── createdAt: timestamp
    │
...
**檔案：** `js/modules/testimonials.js`

**功能描述：**
- 記錄自己、客戶、或從別人那裡分享來的產品使用見證
- 以「改善的問題」和「使用的產品」做分類與交叉檢索
- 跟客戶聊天時能快速找到相關見證
- 照片壓縮成 Base64 存 Firestore（每張 ≤ 150KB，單筆見證最多 5 張）
...
**檢索邏輯：**
- 問題標籤 + 產品標籤可交叉篩選（例如：選「皮膚」+「OPC-3」→ 只顯示同時符合的見證）
- 搜尋框搜文字內容（前端 filter）
- 排序：最新建立的在最前面
...
| 欄位 | 必填 | 類型 | 說明 |
|------|------|------|------|
| 改善問題 | ✅ | 多選標籤 | 預設：過敏、睡眠、腸胃、皮膚、心血管、減重 + 可自訂新增 |
| 使用產品 | ✅ | 多選標籤 | 預設：OPC-3、魚油、益生菌、蘆薈汁、益生消化酵素 + 可自訂新增 |
| 見證內容 | ✅ | 文字 | 改善過程、前後差異 |
| 來源 | 選填 | 文字 | 自己 / 客戶名字 / 空白（分享來的不知道名字就留空） |
| 使用時間 | 選填 | 文字 | 大概多久見效（例如：2 週、1 個月） |
| 照片 | 選填 | 圖片 | 0~5 張，選擇後自動壓縮（寬度 ≤ 800px、品質 0.7）轉 Base64 |

**自訂標籤邏輯：**
- 問題分類和產品清單存在 `users/{uid}/profile` 的 `customTags` 欄位
- 預設清單寫死在程式中
- 使用者新增的自訂標籤 merge 到預設清單後面
- 新增見證時，標籤選擇器底部有「＋ 新增標籤」按鈕

**照片處理流程：**
1. 使用者點「選擇照片」→ `<input type="file" accept="image/*">` + `capture="environment"`
2. 選擇後用 Canvas 壓縮：寬度 ≤ 800px、JPEG 品質 0.7
3. 轉為 Base64 字串
4. 預覽縮圖顯示在 Modal 中，可刪除
5. 儲存時 Base64 字串存在 Firestore 文件的 `photos` 陣列中
6. 單張照片壓縮後約 50-150KB，5 張 ≤ 750KB，加上文字欄位不會超過 1MB 文件上限
